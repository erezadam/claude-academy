...
3. **שער אימות-מקור** (`scripts/verify-sources.mjs`) רץ על המאמרים שהשתנו —
   ראו למטה. אם מזהה טכני כלשהו אינו מאומת במקור הרשמי, השער נכשל וה-PR לא מוזג.
4. **אם ה-build ושער האימות ירוקים — ה-PR מתמזג אוטומטית ל-main** (squash) והאתר מתעדכן.
5. נשלח מייל (אם הוגדר — ראו "secrets למייל"): הצלחה / "אין עדכונים" / כשל.
...
ה-watchdog מריץ את אותו שער אימות-מקור לפני מיזוג PR תקוע, כך שגם הוא לא ימזג
תוכן לא-מאומת.

## שער אימות-מקור (`scripts/verify-sources.mjs`)

build ירוק מוכיח שה-markdown מתקמפל — לא שהתוכן נכון. השער הזה חוסם מיזוג של
טענות שאינן נתמכות במקור הרשמי (התקלה שבה מפתח מומצא `sandbox.credentials` הגיע
פעם לאתר). לכל קובץ `knowledge-base/*.md` שהשתנה בריצה:

1. שולף את `source_url` מה-frontmatter. מותר רק host רשמי דרך https (allowlist —
   הגנת SSRF); מקור לא-נגיש או לא-רשמי = כשל.
2. מחלץ מכל קטעי ה-code מזהים טכניים שמוצגים כרשמיים — flags ארוכים, פקודות
   slash, משתני-סביבה, ומפתחות-קונפיג מנוקדים — ומוודא שכל אחד מופיע מילולית
   במקור. ולו אחד חסר ⇒ `exit 1` עם דוח של הקובץ, המזהה, וה-URL.

בכשל: ה-PR **לא מוזג** ונשאר פתוח, ומתווסף בו comment עם הטענות שלא אומתו.

הרצה ידנית: `node scripts/verify-sources.mjs knowledge-base/<cat>/<file>.md`
(ללא ארגומנטים — נגזרת אוטומטית מהשינויים מול `origin/main`).
