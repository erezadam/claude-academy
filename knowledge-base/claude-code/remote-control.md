...
4. הפעל ב-`/config`:
   - **Push when Claude decides** — התראות יזומות
   - **Push when actions required** — כשנדרש אישור

## אבטחה

הsession שולח רק HTTPS outbound ולא פותח פורטים נכנסים. כל תעבורה עוברת דרך Anthropic API על TLS. Claude Code לא שולח קבצים מקומיים לענן.
