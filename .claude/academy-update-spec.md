...
published: "<חותמת ISO 8601 מלאה עם אזור זמן — נקבעת פעם אחת ביצירה ולעולם לא משתנה; במאמרים היסטוריים נגזרה מהקומיט הראשון>"

הערת חותמות: ‏published ו-body_changed_at נגזרים מ-git ונושאים חותמת מלאה
אמיתית. ‏last_reviewed מוזן ידנית כ-"YYYY-MM-DDT12:00:00+03:00" — רכיב
השעה אינו נושא משמעות; הוא עוגן אזור-זמן ישראלי בלבד, שמונע החלקת יום
בין אזורי זמן. התצוגה לקורא היא תמיד תאריך בלבד.
pathOrder: <ר' 5.4 — רק אם type אינו reference>
---

**אסור בהחלט לכתוב last_reviewed.**

### מעבר היציאה מ-needs-review — סקירה אנושית בלבד
הצינור לעולם אינו מוציא מאמר מ-needs-review. ההוצאה נעשית רק בפעולה אנושית
מפורשת של הבעלים, פר-מאמר: קריאת המאמר במלואו ואימות טענותיו מול המקור
בסטנדרט שלב 6.5. סוכן רשאי להכין סקירה ולדווח ממצאים — אך החתימה עצמה
אנושית. **איסור מוחלט על חתימה בכמות: אין סקריפט שחותם על יותר ממאמר אחד.**
בסיום סקירה, ורק אז: `status: current`, ‏`last_reviewed: <תאריך היום>`,
ורשומה תואמת ב-`reviews.jsonl` בשורש הריפו (‏slug, ‏date, ‏reviewer, ‏note) —
קובץ זה הוא מקור האמת, ו-CI ‏(scripts/check-reviews.mjs) מפיל את הבילד על
כל אי-התאמה בין last_reviewed במאמר לרשומה האחרונה שם.

כשהצינור נוגע שוב במאמר שכבר נסקר: מחזיר `status: needs-review`, מעדכן
`last_verified`, מטביע `body_changed_at: <תאריך היום>` אם גוף המאמר השתנה —
**ואינו נוגע ב-last_reviewed.** באתר: ‏body_changed_at מאוחר מ-last_reviewed
מציג "נסקר ידנית ב-X · עודכן לאחר מכן". השדה ידני ומשמעותו "אדם קרא ואימת טענות"; מאמר אוטומטי נולד בלעדיו, מוצג באתר עם "נוצר אוטומטית — טרם נסקר", ונכנס לתור ההתיישנות. חתימה אוטומטית שלו שוברת את מנגנון הביקורת.
